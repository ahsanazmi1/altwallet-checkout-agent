...
def recommend(
    ...
    category: str = typer.Option("general", "--category", "-c", help="Purchase category (e.g., groceries, travel, gas)"),
    ...
    try:
        ...
        # Display results
        display_recommendation(purchase, best_card, cashback_amount, effective_rate, bonus_text)
...
def demo(
    ...
    p_approval: float = typer.Option(0.95, "--p-approval", help="Probability approval multiplier (0-1, default 0.95)"),
    geo_promo: bool = typer.Option(False, "--geo-promo", help="Apply geographic promotion bonus"),
):
...
def get_demo_card(merchant: str) -> str:
    ...
    elif any(word in merchant_lower for word in ['travel', 'airline', 'hotel', 'booking']):
...
def display_demo_result(recommended_card: str, score: float, signals: dict, verbose: bool):
...
def display_recommendation(purchase: Purchase, card, cashback_amount: Decimal, effective_rate: Decimal, bonus_text: str):
    ...
    if card.category_bonus:
        table.add_row("Category Bonus", f"{card.category_bonus.title()} ({card.category_multiplier}x)")
