...
class AltWalletClient:
    ...
    def _get_headers(self) -> Dict[str, str]:
        ...
        headers = {
            ...
            "User-Agent": f"altwallet-python-sdk/1.0.0",
    ...
    async def _test_connection(self) -> None:
        ...
        try:
            ...
            if response.status_code != 200:
                raise NetworkError(f"Health check failed with status {response.status_code}")
        ...
        for attempt in range(self.config.retry_attempts + 1):
            try:
                ...
                if response.status_code == 200:
                    ...
                    raise APIError(
                        error_data.get("error_message", f"API error: {response.status_code}"),
                        status_code=response.status_code,
                        error_code=error_data.get("error_code"),
                        request_id=request_id,
                        details=error_data.get("details")
    ...
    async def quote(
        self,
        cart: Union[Dict[str, Any], Any],
        customer: Union[Dict[str, Any], Any],
    ...
    async def decision(
        ...
        try:
            # Create request object
            decision_request = DecisionRequest(request_id=request_id)
            
            # Make API request
            response_data = await self._make_request(
                "GET",
                f"/v1/decision/{request_id}",
                request_id=request_id
            )
...
async def create_client(config: Optional[Union[SDKConfig, Dict[str, Any]]] = None) -> AltWalletClient:
