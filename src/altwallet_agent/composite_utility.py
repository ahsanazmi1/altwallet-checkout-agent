...
class CompositeUtility:
    ...
    def calculate_card_utility(
        ...
        try:
            ...
            preference_weight = self.preference_weighting.preference_weight(card, context)
            ...
            utility_breakdown = {
                ...
                "context_info": {
                    "merchant_name": context.merchant.name if context.merchant else "Unknown",
                    "merchant_mcc": context.merchant.mcc if context.merchant else "Unknown",
            ...
            logger.error(f"Error calculating utility for card {card.get('name', 'Unknown')}: {e}")
    ...
    def _calculate_expected_rewards(self, card: Dict[str, Any], context: Context) -> float:
    ...
    def analyze_utility_components(
        ...
        expected_rewards = [card["components"]["expected_rewards"] for card in ranked_cards]
        preference_weights = [card["components"]["preference_weight"] for card in ranked_cards]
        merchant_penalties = [card["components"]["merchant_penalty"] for card in ranked_cards]
        ...
        analysis = {
            ...
            "component_ranges": {
                ...
                "expected_rewards": {"min": min(expected_rewards), "max": max(expected_rewards)},
                "preference_weight": {"min": min(preference_weights), "max": max(preference_weights)},
                "merchant_penalty": {"min": min(merchant_penalties), "max": max(merchant_penalties)},
            },
