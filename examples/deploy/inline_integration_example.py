...
class MerchantCheckoutApp:
    ...
    def __init__(self):
        self.checkout_client: InlineCheckoutClient | None = None
    ...
    async def process_payment(self, payment_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            print(
                f"   Best card: {result['best_card']['card_name'] if result['best_card'] else 'None'}"
            )
            print(
                f"   Approval probability: {result['best_card']['approval_probability']:.2%}"
                if result["best_card"]
                else ""
            )
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "healthy", "altwallet": health, "merchant_app": "running"}
        except Exception as e:
...
class SyncMerchantCheckoutApp:
    ...
    def __init__(self):
        self.checkout_client: SyncInlineCheckoutClient | None = None
        self.initialized = False
...
async def context_manager_example():
    ...
    async with inline_checkout_client(inline_config) as client:
        ...
        request = CheckoutRequest(
            merchant_id="test-merchant", amount=100.0, currency="USD"
        )
...
async def convenience_function_example():
    ...
    request = CheckoutRequest(
        merchant_id="convenience-merchant", amount=50.0, currency="USD"
    )
