...
def demo_travel_vs_grocery_rank_shifts():
    ...
    travel_context = create_sample_context(
        mcc="4511", merchant_name="Delta Airlines"
    )
    
    # Grocery scenario
    grocery_context = create_sample_context(
...
def demo_composite_utility_formula():
    ...
    print(f"Formula: utility = p_approval × expected_rewards × preference_weight × merchant_penalty")
    print()
    ...
    print()
    print(f"Calculation:")
    print(f"  utility = {p_approval:.3f} × {expected_rewards:.3f} × {preference_weight:.3f} × {merchant_penalty:.3f}")
...
def main():
    ...
    try:
        ...
        print("\nKey Insights:")
        print("- Utility combines approval probability, rewards, preferences, and penalties")
        print("- Rank shifts occur based on MCC categories, loyalty tiers, and merchant preferences")
        print("- The formula ensures optimal card selection for each transaction context")
