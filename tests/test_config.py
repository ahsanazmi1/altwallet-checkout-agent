...
class TestOrcaConfig:
    ...
    def test_legacy_fallback_with_deprecation_warning(self):
        ...
        with patch.dict(
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                assert len(w) == 2
                assert any(
                    "ALTWALLET_API_KEY is deprecated" in str(warning.message)
                    for warning in w
                )
                assert any(
                    "ALTWALLET_ENDPOINT is deprecated" in str(warning.message)
                    for warning in w
                )
    ...
    def test_deprecation_warning_only_issued_once(self):
        ...
        with patch.dict(os.environ, {"ALTWALLET_API_KEY": "legacy_key"}, clear=True):
            config = OrcaConfig()
