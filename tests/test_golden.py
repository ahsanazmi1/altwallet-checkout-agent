...
class GoldenRegressionTest:
    ...
    def _extract_feature_drivers(self, attributions) -> List[Dict[str, Any]]:
        ...
        for attr_name, value in vars(attributions).items():
            if attr_name.startswith('_') or attr_name in ['baseline', 'contribs', 'sum']:
                continue
    ...
    def _extract_risk_signals(self, context: Context) -> Dict[str, bool]:
        ...
        return {
            ...
            "cross_border": getattr(context, 'cross_border', False),
            "high_velocity": getattr(context, 'velocity_24h', 0) > 10 or getattr(context, 'velocity_7d', 0) > 50,
    ...
    def test_fixture(self, fixture_name: str):
        ...
        assert actual_result["p_approval"] == expected_snapshot["p_approval"], \
            f"p_approval mismatch for {fixture_name}"
        
        assert actual_result["top_card"] == expected_snapshot["top_card"], \
            f"top_card mismatch for {fixture_name}"
        
        assert abs(actual_result["utility"] - expected_snapshot["utility"]) < 0.001, \
            f"utility mismatch for {fixture_name}"
        
        assert actual_result["raw_score"] == expected_snapshot["raw_score"], \
            f"raw_score mismatch for {fixture_name}"
        
        assert actual_result["calibration_method"] == expected_snapshot["calibration_method"], \
...
if __name__ == "__main__":
    ...
    fixture_names = [
        'grocery_small_ticket', 'travel_high_value', 'cross_border',
        'device_mismatch_high_velocity', 'loyalty_gold_with_promo',
        'merchant_prefers_debit', 'high_risk_mcc', 'missing_fields',
        'issuer_affinity', 'mixed_cart_large_amount'
    ]
    for i in range(1, 11):
