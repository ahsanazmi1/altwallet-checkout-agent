...
class TestRoutingHint:
    ...
    def test_routing_hint_computed_fields(self):
        ...
        hint_no_penalty = RoutingHint(penalty_or_incentive=PenaltyOrIncentive.NONE)
...
class TestDecisionEngine:
    ...
    def test_penalty_or_incentive_calculation(self):
        ...
        penalty = engine._calculate_penalty_or_incentive(context, high_risk_score)
        assert penalty == PenaltyOrIncentive.SURCHARGE

        # Test premium customer (should get suppression)
        premium_context = self._create_test_context(loyalty_tier=LoyaltyTier.GOLD)
        ...
        penalty = engine._calculate_penalty_or_incentive(premium_context, normal_score)
        ...
        penalty = engine._calculate_penalty_or_incentive(normal_context, normal_score)
    ...
    def _create_test_context(self, loyalty_tier=LoyaltyTier.SILVER):
        ...
        cart_items = [
            CartItem(item="Test Item", unit_price="100.00", qty=1, mcc="5411")
        ]

        cart = Cart(items=cart_items, currency="USD")

        merchant = Merchant(
            name="Test Merchant", mcc="5411", network_preferences=["visa"]
        )
...
class TestIntegration:
    ...
    def test_full_decision_flow(self):
        ...
        assert (
            quick_decision.decision == decision.decision
