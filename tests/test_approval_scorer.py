...
class TestMonotonicity:
    ...
    def test_extreme_risk_scenarios(self):
        ...
        assert (
            result.p_approval < 0.1
        ), f"Extreme risk scenario should have very low approval probability, got {result.p_approval}"
        assert (
            result.raw_score < -5.0
        ), f"Extreme risk scenario should have very negative raw score, got {result.raw_score}"
