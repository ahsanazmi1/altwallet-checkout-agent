...
def normalize_json(data: dict[str, Any]) -> dict[str, Any]:
    ...
    dynamic_fields = ["trace_id", "transaction_id", "request_id", "processing_time_ms"]
...
class TestOrcaGoldenScenarios:
    ...
    def test_orca_online_grocery_golden(self):
        ...
        actual_normalized = normalize_json(actual_output)
        expected_normalized = normalize_json(expected_output)
        ...
        assert actual_hints["interchange_optimization"] == expected_hints["interchange_optimization"]
        assert actual_hints["surcharge_suppression"] == expected_hints["surcharge_suppression"]
        ...
        assert len(actual_output["recommendations"]) == len(expected_output["recommendations"])
    ...
    def test_all_orca_golden_fixtures(self):
        ...
        for fixture_name in orca_fixture_files:
            ...
            assert actual_output["decision"] in ["APPROVE", "REVIEW", "DECLINE"]
