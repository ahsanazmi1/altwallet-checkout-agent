...
class TestAPIModels:
    ...
    def test_decision_response_model(self):
        ...
        from altwallet_agent.decisioning import DecisionContract, Decision, BusinessRule, DecisionReason, ActionType, RoutingHint
        
        decision_contract = DecisionContract(
            decision=Decision.APPROVE,
            actions=[BusinessRule(rule_id="test", action_type=ActionType.LOYALTY_BOOST, description="Test")],
            reasons=[DecisionReason(feature_name="test", value="good", threshold=0.5, weight=0.8, description="Test")],
...
class TestAPIEndpoints:
    ...
    def test_score_endpoint_success(self, sample_context_data):
        ...
        with patch('altwallet_agent.api.composite_utility') as mock_utility:
            with patch('altwallet_agent.data.card_database.CardDatabase') as mock_card_db:
                # Mock card database
...
class TestAPIErrorHandling:
    ...
    def test_score_endpoint_processing_error(self):
        ...
        with patch('altwallet_agent.api.Context.from_json_payload', side_effect=Exception("Processing error")):
            response = client.post("/v1/score", json={
                "context_data": {"valid": "data"}
            })
            ...
            with patch('builtins.open', mock_open()) as mock_file:
                with patch('json.dump') as mock_json_dump:
                    with patch('altwallet_agent.api.get_logger') as mock_logger:
                        ...
                        mock_schema_path.__str__ = Mock(return_value="openapi/openapi.json")
                ...
                with patch('altwallet_agent.api.score_transaction') as mock_score:
                    with patch('altwallet_agent.api.decision_engine') as mock_engine:
                        ...
                        with patch('altwallet_agent.api.Context.from_json_payload') as mock_context:
                            ...
                            explain_response = client.post("/v1/explain", json={
                                "context_data": context_data
                            })
                            assert explain_response.status_code == 200
                        
                        # Test decision endpoint
                        decision_response = client.post("/v1/decision", json={
                            "context_data": context_data
                        })
