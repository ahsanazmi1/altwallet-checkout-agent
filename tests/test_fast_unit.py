...
class TestCalibrationMonotonicity:
    ...
    def test_calibrator_bounds(self):
        ...
        for score in extreme_scores:
            ...
            assert 0.0 <= prob <= 1.0, f"Probability {prob} not in [0,1] for score {score}"
...
class TestPreferenceWeights:
    ...
    def test_issuer_affinity_weights(self):
        ...
        card_amex = {"name": "Amex Card", "issuer": "american_express", "rewards_type": "points"}
    ...
    def _create_test_context(self, loyalty_tier: LoyaltyTier, 
                            issuer_affinity: str = "chase") -> Context:
        ...
        return Context(
            ...
            device=Device(ip="192.168.1.100", location={"city": "Test", "country": "US"}),
...
class TestMerchantPenalties:
    ...
    def test_network_preference_penalties(self):
        ...
        penalty_non_preferred = self.penalty.merchant_penalty(
            context_non_preferred
        )
    ...
    def _create_test_context(self, network_preferences=None) -> Context:
        ...
        return Context(
            ...
            device=Device(ip="192.168.1.101", location={"city": "Test", "country": "US"}),
            geo=Geo(city="Test", country="US"),
        )
...
class TestAttributionAdditivity:
    ...
    def test_attribution_consistency(self):
        ...
        assert result1.additive_attributions.sum == result2.additive_attributions.sum, \
    ...
    def test_attribution_sensitivity(self):
        ...
        assert high_risk_result.raw_score < base_result.raw_score, \
            "High-risk context should have lower score"
        
        # MCC attribution should be different
        base_mcc_contrib = next(
            c.value for c in base_result.additive_attributions.contribs 
            if "mcc" in c.feature.lower()
        )
        high_risk_mcc_contrib = next(
            c.value for c in high_risk_result.additive_attributions.contribs 
            if "mcc" in c.feature.lower()
        )
        
        assert base_mcc_contrib != high_risk_mcc_contrib, \
