...
class DeploymentManager(DeploymentProvider):
    ...
    def get_mode_config(self) -> SidecarConfig | InlineConfig:
    ...
    def create_inline_client(
        self,
    ) -> InlineCheckoutClient | SyncInlineCheckoutClient:
    ...
    def validate_configuration(self) -> dict[str, Any]:
        ...
        if self.config.api_port < 1 or self.config.api_port > 65535:
            ...
            validation_results["errors"].append(
                f"Invalid API port: {self.config.api_port}"
            )

        # Validate worker configuration
        if self.config.max_workers < 1:
            validation_results["warnings"].append(
                f"Low max_workers value: {self.config.max_workers}"
            )

        # Validate timeout configuration
        if self.config.request_timeout < 1:
            validation_results["warnings"].append(
                f"Low request_timeout value: {self.config.request_timeout}"
            )
