...
class DeploymentConfig(BaseModel):
    ...
    mode: DeploymentMode = Field(
        default=DeploymentMode.SIDECAR,
        description="Deployment mode: sidecar or inline"
    )
    ...
    sidecar_config: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Sidecar-specific configuration"
    )
    
    inline_config: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Inline-specific configuration"
    )
...
class SidecarConfig(BaseModel):
    ...
    config_volume: Optional[str] = Field(
        default=None,
        description="Configuration volume mount"
    )
    
    log_volume: Optional[str] = Field(
        default=None,
        description="Log volume mount"
    )
...
class InlineConfig(BaseModel):
    ...
    merchant_app_name: str = Field(
        default="merchant-app",
        description="Name of the merchant application"
    )
    ...
    connection_pool_size: int = Field(
        default=10,
        description="Connection pool size for HTTP client"
    )
