...
class InlineCheckoutClient:
    ...
    def __init__(self, config: InlineConfig | None = None):
        ...
        self._agent: CheckoutAgent | None = None
    ...
    async def process_checkout(
        self,
        request: CheckoutRequest | dict[str, Any],
    ...
    async def _process_with_retry(
        self, request: CheckoutRequest, context: Context | None = None
    ) -> CheckoutResponse:
    ...
    def _is_circuit_breaker_open(self) -> bool:
        ...
        if self._circuit_breaker_state == "open":
            ...
            if (
                time.time() - self._circuit_breaker_last_failure > 60
            ):  # 1 minute timeout
...
_inline_client: InlineCheckoutClient | None = None
...
async def process_checkout_inline(
    request: CheckoutRequest | dict[str, Any], context: Context | None = None
) -> CheckoutResponse:
...
@asynccontextmanager
async def inline_checkout_client(config: InlineConfig | None = None):
...
class SyncInlineCheckoutClient:
    ...
    def process_checkout(
        ...
        loop = self._get_loop()
        return loop.run_until_complete(self._client.process_checkout(request, context))
